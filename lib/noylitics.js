...
Noylitics.Handler.prototype = {
    ...
    handleRatingEvent: function(identifier,value) {
	sys.puts("Handle event");
	for(subscriber in this.ratingSubscribers)
	{
	    this.ratingSubscribers[subscriber].sendHeader(200, { "Content-Type" : "text/plain" });
	    
	    sys.puts("streaming: " + JSON.stringify({"identifier":identifier,"value":value}));
	    this.ratingSubscribers[subscriber].write(JSON.stringify({"identifier":identifier,"value":value}));
	    sys.puts("streamed: " + JSON.stringify({"identifier":identifier,"value":value}));
	    
	    this.ratingSubscribers[subscriber].close();
	}
	this.ratingSubscribers = [];
    },
	...
	{
	    this.ratingSubscribers.push(res);
	    return true
	}
      return false;
    }
