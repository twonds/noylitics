...
Statistics.prototype.handleRating = function(identifier, rating) {
    sys.puts("*******************RATING: " + sys.inspect(rating));
	
    if(!this.raters.hasOwnProperty(identifier)) {
	this.raters[identifier] = new stats.Rater();
    }
    
    this.raters[identifier].addRating(identifier, rating);
    this.emit('rating', identifier, this.raters[identifier].value(identifier));
}
